...
# def pdf_contrat(request, pk):
#     # Récupération correcte : ton modèle utilise pk_contrat !
#     contrat = get_object_or_404(ContratTransport, pk_contrat=pk)
...
#     styles.add(ParagraphStyle(name="Label", fontSize=12, leading=14))
#     styles.add(ParagraphStyle(name="NormalJustify", alignment=4, fontSize=11, leading=14))

#     story = []

#     # 🔹 Titre
#     story.append(Paragraph("<b>CONTRAT DE TRANSPORT</b>", styles["TitleCenter"]))
#     story.append(Spacer(1, 10))

#     # 🔹 Bloc principal
#     story.append(Paragraph(f"<b>Numéro BL :</b> {contrat.numero_bl}", styles["Label"]))
#     story.append(Paragraph(f"<b>Destinataire :</b> {contrat.destinataire}", styles["Label"]))
#     story.append(Paragraph(f"<b>Conteneur :</b> {contrat.conteneur.numero_conteneur}", styles["Label"]))
#     story.append(Paragraph(f"<b>Chauffeur :</b> {contrat.chauffeur.nom} {contrat.chauffeur.prenom}", styles["Label"]))
#     story.append(Paragraph(f"<b>Camion :</b> {contrat.camion.immatriculation}", styles["Label"]))
#     story.append(Paragraph(f"<b>Date début :</b> {contrat.date_debut}", styles["Label"]))
#     story.append(Paragraph(f"<b>Date limite retour :</b> {contrat.date_limite_retour}", styles["Label"]))
#     story.append(Spacer(1, 12))
...
#         buffer,
#         content_type="application/pdf",
#         headers={"Content-Disposition": f'attachment; filename=\"Contrat_{pk}.pdf\"'}
#     )
